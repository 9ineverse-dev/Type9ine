...
>
	...
	<div v-if="isRenote" :class="$style.renote">
		...
		<div :class="$style.renoteInfo">
			...
			><i class="ti ti-rocket-off"></i></span>
			<span
				v-if="note.channel"
				style="margin-left: 0.5em"
				:title="note.channel.name"
			><i class="ti ti-device-tv"></i></span>
		</div>
	</div>
...
<div
	v-else-if="muted && !hideMutedNotes"
	:class="$style.muted"
	@click="muted = false"
>
	<I18n
		v-if="muted === 'sensitiveMute'"
...
function toggleReact() {
	if (
		appearNote.value.myReactions?.length < 4 ||
		(appearNote.value.myReaction && appearNote.value.user.host) ||
		!appearNote.value.myReactions
	) {
		react();
	}
}
...
.root {
	...
	&.home {
		background-color: rgba(var(--homeColor), 0.2) !important;
	}

	&.followers {
		background-color: rgba(var(--followerColor), 0.2) !important;
	}

	&.specified {
		background-color: rgba(var(--specifiedColor), 0.2) !important;
	}

	&.localonly {
		background-color: rgba(var(--localOnlyColor), 0.2) !important;
	}
...
.root:has(.ti-home) {
	background-color: rgba(255, 255, 100, 0.1) !important;
}
</style>
