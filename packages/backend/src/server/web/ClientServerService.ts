...
export class ClientServerService {
	...
	private async generateCommonPugData(meta: MiMeta) {
		return {
			instanceName: meta.name ?? 'Type4ny',
			icon: meta.iconUrl,
	...
	public createServer(
		...
		fastify.get<{
			Querystring: { url: string; lang: string };
		}>('/url', (request, reply) =>
			this.urlPreviewService.handle(request, reply),
		);
