...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	constructor(
		...
		if (followees.length > 0 /*|| distinctRns.length > 0*/) {
			...
			.orWhere('(note.id IN (SELECT max_id from (SELECT MAX(note.id) max_id FROM note WHERE ((note.userId IN (:...meOrFolloweeIds)) AND ((note.userHost = renote.userHost) OR (note.userHost IS NULL)) AND (renote.remnoteCount > 3 ) ) GROUP BY note.renoteId LIMIT 100) temp)',{ meOrFolloweeIds: meOrFolloweeIds})
			.orWhere('(note.id IN (SELECT max_id from (SELECT MAX(note.id) max_id FROM note WHERE ((note.userId IN (:...meOrFolloweeIds)) AND (renote.userId IN (:...meOrFolloweeIds)) AND (renote.userHost IS NULL) AND (note.userHost IS NULL) AND (renote.remnoteCount > 2 )  ) GROUP BY note.renoteId LIMIT 100) temp)',{ meOrFolloweeIds: meOrFolloweeIds});
		  }));
		} else {
