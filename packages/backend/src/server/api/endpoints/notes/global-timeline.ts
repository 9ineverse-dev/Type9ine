...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	constructor(
		...
		if (me.followingCount > 0 ) {
			...
			.orWhere(`((renote.renoteCount > :renoteCounter3) AND (note.userId != renote.userId) AND (note.id IN (SELECT DISTINCT ON (note.renoteId) note.id FROM note WHERE ((note.id > :minrnId)  AND (note.userId IN (${ followingQuery.getQuery() })))ORDER BY note.id DESC)))`,{renoteCounter3:DynamicRenoteCount3,minrnId: this.idService.genId(new Date(Date.now() - (1000 * 60 * 60 * 24 * 4)))});
		  }));
		} else {
			query.andWhere(new Brackets(qb =>{
