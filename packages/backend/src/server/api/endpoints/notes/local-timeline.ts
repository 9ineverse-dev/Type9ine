			...
			const query = this.queryService.makePaginationQuery(this.notesRepository.createQueryBuilder('note'),
				...
				.andWhere(new Brackets(qb => {
					qb.where(`((note.userId IN (${ followingQuery.getQuery() })) AND (note.renoteCount > :minrenoteCount1) AND (note.renote IS NULL))`,{minrenoteCount1: DynamicRenoteCount1})
						...
						.orWhere(`((note.userId IN (${ followingQuery.getQuery() })) AND (note.renoteId IS NOT NULL) AND (note.userHost IS NULL) AND (renote.userId IN (${ followingQuery.getQuery() })) AND (renote.fileIds IS NOT NULL) AND (renote.userHost IS NULL) AND (renote.renoteCount > :minrenoteCount5)) AND (note.id IN (SELECT max_id from (SELECT MAX(note.id) max_id FROM note WHERE ((note.userId IN (${ followingQuery.getQuery() })) AND (note.userHost IS NULL)) GROUP BY note.renoteId) temp))`,{minrenoteCount5: DynamicRenoteCount5});
				}))
