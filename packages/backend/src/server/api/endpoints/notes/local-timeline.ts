			...
			const query = this.queryService.makePaginationQuery(this.notesRepository.createQueryBuilder('note'),
				...
				.andWhere(new Brackets(qb => {
					qb.where(`((note.userId IN (${ followingQuery.getQuery() })) AND (note.renoteCount > :minrenoteCount1) AND (note.renote IS NULL))`,{minrenoteCount1: 5})
						...
						.orWhere(`((note.userId IN (${ followingQuery.getQuery() })) AND NOT(note.renote!.userId IN (${ followingQuery.getQuery() })) AND (note.renote.renoteCount >= :minrenoteCount4))`,{minrenoteCount4: 10});
				}))
